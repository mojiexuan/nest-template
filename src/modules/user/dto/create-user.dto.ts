...
export class CreateUserDto {
  ...
  @ApiProperty({ description: '密码', example: '123456', minLength: 6, maxLength: 20 })
  @IsString({ message: '密码必须是字符串' })
  @MinLength(6, { message: '密码长度不能少于6个字符' })
  @MaxLength(20, { message: '密码长度不能超过20个字符' })
  password: string;

  @ApiPropertyOptional({ description: '昵称', example: 'John', maxLength: 100 })
  @IsOptional()
  @IsString({ message: '昵称必须是字符串' })
  @MaxLength(100, { message: '昵称长度不能超过100个字符' })
  nickname?: string;

  @ApiPropertyOptional({ description: '微信OpenID', maxLength: 100 })
  @IsOptional()
  @IsString({ message: 'OpenID必须是字符串' })
  @MaxLength(100, { message: 'OpenID长度不能超过100个字符' })
  openid?: string;

  @ApiPropertyOptional({ description: '用户类型', enum: UserType, default: UserType.STUDENT })
  @IsOptional()
